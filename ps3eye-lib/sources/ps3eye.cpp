...
static const uint8_t ov772x_reg_initdata[][2] = {
	...
	{ 0x12, 0x03 },		/* 0x01 - Processed Bayer RAW (8bit) , 0x00 - YUV *, 0x03 - Byer RAW */
...
static const uint8_t bridge_start_vga[][2] = {
	{0x1c, 0x00},
	{0x1d, 0x00},  /* correct - 0x00 RAW8 !!!!!!!!!!!!!! works also YUV 0x40 !!!! */
	{0x1d, 0x02},
	{0x1d, 0x00},
	{0x1d, 0x01},	/* frame size = 0x012C00 * 4 = 307200 bytes (640 * 480 @ 8bpp) */
	{0x1d, 0x2C},	/* frame size */
...
static const uint8_t sensor_start_vga[][2] = {
	{0x12, 0x03}, /* 0x01 Processed Bayer raw 8, 0x00 YUV*, 0x03 - byer raw*/
	{0x17, 0x26},
	{0x18, 0xa0},
	{0x19, 0x07},
	{0x1a, 0xf0},
...
void USBMgr::transferThreadFunc()
{
	...
	while (!exit_signaled)
	{
		...
		{
			dataSize += usb_reap_async(urb->xfs[index], 5000);
			usb_submit_async(urb->xfs[index], (char *)urb->transfer_buffer + index * TRANSFER_SIZE, TRANSFER_SIZE);
			urb->pkt_scan((uint8_t*)urb->transfer_buffer, dataSize);
		}
...
int USBMgr::listDevices( std::vector<PS3EYECam::PS3EYERef>& list )
{
	...
	if (usb_find_busses() < 0) {
		debug("usb_find_busses() error");
		return cnt;
	}

	if (usb_find_devices() < 0) {
		debug("usb_find_devices() error");
		return cnt;
	}

	for (bus = usb_busses; bus; bus = bus->next) {
		for (dev = bus->devices; dev; dev = dev->next) {
			if(dev->descriptor.idVendor==PS3EYECam::VENDOR_ID &&
			   dev->descriptor.idProduct==PS3EYECam::PRODUCT_ID) {
				if ((devhandle = usb_open(dev)) != NULL) {
					usb_close(devhandle);
...
public:
	...
	bool start_transfers(struct usb_dev_handle *handle, uint32_t curr_frame_size)
	{
		...
		for (int index = 0; index < NUM_TRANSFERS; ++index)
		{
			...
			*/
			usb_bulk_setup_async(handle, &xfr[index], BULK_ENDPOINT);

			// res |= libusb_submit_transfer(xfr[index]);
			
			usb_submit_async(xfr[index], (char *)transfer_buffer + index * TRANSFER_SIZE, TRANSFER_SIZE);
	...
	void close_transfers()
	{
		...
		for (int index = 0; index < NUM_TRANSFERS; ++index)
		{
			//printf("usb_cancel_async(xfr[index]);");
			usb_cancel_async(xfr[index]);
			//usb_free_async(&xfr[index]);
	...
	void pkt_scan(uint8_t *data, int len)
	{
	    ...
	    do {
	        ...
	        else if (data[1] & UVC_STREAM_EOF) 
	        {
                ...
                {
                    //printf("packet was discarded. cur_frame_data_len + len - 12 = %d, frame_size = %d", cur_frame_data_len + len - 12, frame_size);
					goto discard;
                }
...
private:
	...
	void startTransferThread()
	{
		update_thread = std::thread(&URBDesc::transferThreadFunc, this);
		//printf("transfer thread started: %d", update_thread.get_id());
	}
	...
	void transferThreadFunc()
	{
		...
		while (!exit_signaled)
		{
			...
			for (int index = 0; index < NUM_TRANSFERS; ++index)
			{
				dataSize = usb_reap_async(xfr[index], 5000);
				usb_submit_async(xfr[index], (char *)transfer_buffer + index * TRANSFER_SIZE, TRANSFER_SIZE);
				pkt_scan((uint8_t*)transfer_buffer + index * TRANSFER_SIZE, dataSize);
			}
...
PS3EYECam::PS3EYECam(struct usb_device *device)
{
	// default controls
	/*
	autogain = true;
...
bool PS3EYECam::open_usb()
{
	...
	// res = libusb_open(device_, &handle_);
	/* if(res != 0) {
		debug("device open error: %d\n", res);
		return false;
	} */
	
	if((handle_ = usb_open(device_)) == NULL) {
		debug("device open error: %d\n", res);
		return false;
	}
	...
	res = usb_claim_interface(handle_, 0);
	if(res != 0) {
...
void PS3EYECam::close_usb()
{
	...
	*/
	...
	usb_release_interface(handle_, 0);
	usb_close(handle_);
